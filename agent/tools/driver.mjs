...
const ctx = await browser.newContext({
  ...
  ...(process.env.DRIVER_UA ? { userAgent: process.env.DRIVER_UA } : {}),
...
const ACT = {
  ...
  lastTab: async () => { const ps = ctx.pages(); page = ps[ps.length - 1]; await page.bringToFront().catch(()=>{}); say("tab -> " + page.url().slice(0,100)); },
  firstTab: async () => { const ps = ctx.pages(); page = ps[0]; await page.bringToFront().catch(()=>{}); say("tab -> " + page.url().slice(0,100)); },
  closeTab: async () => { const ps = ctx.pages(); if (ps.length > 1) { await page.close().catch(()=>{}); const ps2 = ctx.pages(); page = ps2[ps2.length - 1]; } say("tabs=" + ctx.pages().length); },
  ...
  clickFrame: async (a) => {
    const f = page.frames().find((fr) => new RegExp(a.frameRe).test(fr.url()));
    if (!f) throw new Error("no frame matching " + a.frameRe);
    await f.locator(a.sel).first().click({ timeout: 10000 });
  },
  fillFrame: async (a) => {
    const f = page.frames().find((fr) => new RegExp(a.frameRe).test(fr.url()));
    if (!f) throw new Error("no frame matching " + a.frameRe);
    await f.locator(a.sel).first().fill(a.value, { timeout: 8000 });
  },
  fillFrameEnv: async (a) => {
    const f = page.frames().find((fr) => new RegExp(a.frameRe).test(fr.url()));
    if (!f) throw new Error("no frame matching " + a.frameRe);
    const v = process.env[a.env] ?? "";
    await f.locator(a.sel).first().fill(v, { timeout: 8000 });
    say(`fillFrameEnv ${a.sel} <- ${a.env} (${v ? "set" : "EMPTY"})`);
  },
  uploadFrame: async (a) => {
    const f = page.frames().find((fr) => new RegExp(a.frameRe).test(fr.url()));
    if (!f) throw new Error("no frame matching " + a.frameRe);
    await f.locator(a.sel ?? 'input[type="file"]').first().setInputFiles(a.path);
  },
  netlog: async () => {
    globalThis.__net = [];
    page.on("response", async (res) => {
      const u = res.url();
      if (/wday|workday|api/.test(u) && res.request().method() !== "GET") {
        let body = "";
        try { body = (await res.text()).slice(0, 500); } catch {}
        globalThis.__net.push(`${res.status()} ${res.request().method()} ${u.slice(0, 140)} :: ${body}`);
      }
    });
    say("netlog armed");
  },
  netdump: async () => { say((globalThis.__net || []).slice(-8).join("\n") || "no captures"); },
};
...
while (Date.now() < deadline) {
  ...
  for (const a of batch.actions ?? []) {
    if (page.isClosed()) { const ps = ctx.pages(); page = ps[ps.length - 1] ?? await ctx.newPage(); say("page recovered -> " + page.url().slice(0,80)); }
    try { await ACT[a.do](a); say(`ok: ${a.do} ${a.sel ?? a.name ?? a.label ?? a.button ?? a.url ?? ""}`); }
    catch (e) { say(`FAIL: ${a.do} ${a.sel ?? a.name ?? ""} -> ${e.message.split("\n")[0]}`); }
    if (page.isClosed()) { const ps = ctx.pages(); page = ps[ps.length - 1] ?? await ctx.newPage(); }
    await page.waitForTimeout(a.settleMs ?? 1200).catch(()=>{});
  }
  if (page.isClosed()) { const ps = ctx.pages(); page = ps[ps.length - 1] ?? await ctx.newPage(); }
  await page.waitForTimeout(batch.settleMs ?? 2500).catch(()=>{});
