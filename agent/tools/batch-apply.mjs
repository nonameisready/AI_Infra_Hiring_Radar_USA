  ...
  try {
    ...
    if (/greenhouse\.io$/.test(host) || /greenhouse/.test(host)) {
      ...
      if (rj.confirmation) {
        ...
        const why = [rj.note && `note: ${rj.note}`,
          rj.securityCodePending && "greenhouse is holding an unfinished verification for this posting",
          rj.error && `error: ${rj.error}`, rj.finalUrl && `url: ${rj.finalUrl}`, snippet.slice(0, 120)]
